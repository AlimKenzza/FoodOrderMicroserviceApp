...
func main() {
	l, err := net.Listen("tcp", "0.0.0.0"+":4001")
	if err != nil {
		log.Fatalf("Failed to listen:%v", err)
	}
	s := grpc.NewServer()
	restarauntpb.RegisterFoodOrderServiceServer(s, &Server{})
	log.Println("Server is running on port ", "4001")
...
func requestDispatcher(orderID int32) []*restarauntpb.Food {
	...
	foods = GetOFoodsByOrderId(pool, orderID)
	return foods
}
func GetOFoodsByOrderId(pool *pgxpool.Pool, orderID int32) []*restarauntpb.Food {
